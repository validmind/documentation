...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log('Congratulations, your extension "vmjupyter-toc" is now active!');

    // The command has been defined in the package.json file
    // Now provide the implementation of the command with registerCommand
    // The commandId parameter must match the command field in package.json
    let command1 = vscode.commands.registerCommand('vmjupyter-toc.jupyterToc', () => {
        if (vscode.window.activeNotebookEditor?.notebook !== undefined) {
            console.log("Start building TOC...")
            new TocGenerator().process();
        }
        console.log("Command 'vmjupyter-toc.jupyterToc' executed");
    });

    let command2 = vscode.commands.registerCommand('vmjupyter-toc.jupyterUnToc', () => {
        if (vscode.window.activeNotebookEditor?.notebook !== undefined) {
            console.log("Start removing TOC...")
            new TocGenerator().process(true);
        }
        console.log("Command 'vmjupyter-toc.jupyterUnToc' executed");
    });
...
export class TocGenerator {
    ...
    private anchorHeader(header: Header): string {
        ...
        if (this._config.Anchor) {
            ...
            // Place the anchor above the header while keeping the Markdown intact
            title = `${anchor}\n\n${title}`;
        }
...
class TocConfiguration {
    ...
    public StartLine: string = "<!-- vscode-vmjupyter-toc-config";
    public EndLine: string = "/vscode-vmjupyter-toc-config -->";
