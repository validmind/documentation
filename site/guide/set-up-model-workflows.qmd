...
- To customize workflows, you must hold the `Customer Admin` role
- Resource statuses must be fully configured
...
- Requires available resource statuses associated with the workflow.
...
- [Customize resource statuses](customize-resource-statuses.qmd)
- [Add conditional requirements](#add-conditional-requirements)
:::
...
| Field | Description |
|---:|---|
| Display action button when (optional) | Add conditional requirements for displaying this step.  |
| Action button label (required) | Text that displays on your action button. |
| Request fields (optional) | Include custom fields alongside your action if desired. Select the field, then click **+ Add Field** to insert.  |
...
- Requires a **User Type** custom field to select from, and must be connected to both an approval and rejection Status Change step.
...
- [Manage inventory custom fields](manage-inventory-custom-fields.qmd)
- [Status Change](#status-change) steps
- [Add conditional requirements](#add-conditional-requirements)
:::
...
1. First, configure an **Approval** step. 
