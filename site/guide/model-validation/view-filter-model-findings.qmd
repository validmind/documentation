...
- [x] You are a [{{< fa circle-check >}} Validator]{.role} or assigned another role with sufficient permissions to perform the tasks in this guide.[^3] 
...
1. In the left sidebar, click **{{< fa triangle-exclamation >}} Model Findings**.
   ...
   ![List of findings across all models](all-model-findings.png){width=80% fig-alt="A screenshot showing a list of findings across all models" class="screenshot"}

::: {.callout title="Changes are automatically saved to your account and do not affect other users."}
Filter this list to include only findings you want to see,[^4] or toggle visibilty for column headers that can be dragged and dropped to change the order with the **{{< fa table >}} Manage Columns** menu.
...
3. In the left sidebar that appears for your model, click **Model Findings**.
   ...
   ![List of findings specific to a model](model-specific-findings.png){width=80% fig-alt="A screenshot showing a list of findings specific to a model" class="screenshot"}
...
3. On the **Select Your Filters** dialog that opens, select your filtering criteria for:
   ...
   ![Filter dialog with some options selected](filter.png){width=80% fig-alt="A screenshot of the filter dialog with some options selected" class="screenshot"}
