---
title: "Assess compliance"
date: last-modified
aliases:
  - /guide/assess-compliance.html
---

Use the {{< var validmind.platform >}} to assess compliance of your models with guidelines based on analyzing evidence and findings.

Compliance assessments provide an unbiased starting point that enables more efficient discussions between validators and developers and involves three steps:

1. [Link evidence to reports](#link-evidence-to-reports)
2. [Link findings to reports](#link-findings-to-reports)
3. [Provide compliance assessments](#provide-compliance-assessments)

::: {.attn}

## Prerequisites

- [x] {{< var link.login >}}
- [x] The model you are validating is registered in the model inventory.[^1]
- [x] A model developer has submitted their model documentation for validation.[^2]
- [x] You are a [{{< fa circle-check >}} Validator]{.bubble} or assigned another role with sufficient permissions to perform the tasks in this guide.[^3] 

:::

## Link evidence to reports

### Link developer evidence

To link evidence logged by developers^[[{{< var validmind.developer >}}](/developer/validmind-library.qmd)] to your validation report:

1. In the left sidebar, click **{{< fa cubes >}} Inventory**.

2. Select a model or find your model by applying a filter or searching for it.[^4]

3. In the left sidebar that appears for your model, click **{{< fa file >}} Documents** and select a Validation Report type file.[^5]

   Expand any subsection of the validation report you would like to work with.

   For example, locate 2. Validation then select **2.1.1. Assumptions**.

4. In any section of the report where the button is available, click **{{< fa link >}} Link Developer Evidence**. 

   ![Validation report section 2.1.1. that shows a compliance assessment with the option to link to developer evidence](link-developer-evidence.png){fig-alt="A screenshot of the validation report section 2.1.1. that shows a compliance assessment with the option to link to developer evidence" .screenshot}

5. On the **Link Developer Evidence to Validation Report** page that opens, select the evidence that is related to your assessment. 
   
   - Available evidence is associated with the relevant documentation section. 
   - If you are not sure if something is relevant, you can expand the section by clicking **{{< fa chevron-down >}}** for more details.

6. Click **Update Linked Evidence**.

   - Confirm that the newly linked-to evidence shown under Developer Evidence is accurate and is connected to the correct documentation section.
   - Evidence that needs your attention gets flagged with [{{< fa triangle-exclamation >}} Requires Attention]{.bubble .yellow-bg}. These sections get flagged automatically whenever a test result is above or below a certain threshold.

### Link validator evidence

::: {.callout}
To link validator evidence to a report, you must first log tests as a validator with the {{< var validmind.developer >}}.[^6]
:::

1. In the left sidebar, click **{{< fa cubes >}} Inventory**.

2. Select a model or find your model by applying a filter or searching for it.[^7]

3. In the left sidebar that appears for your model, click **Validation Report** under {{< fa file >}} Documents.[^8]

   You can now expand any subsection of the validation report you would like to work with.

   For example, locate 2. Validation then select **2.1.1. Assumptions**.

4. In any section of the report where the button is available, click **{{< fa link >}} Link Evidence to Report**. 

   ![Validation report section 2.1.1. that shows a compliance assessment with the option to link evidence](/guide/model-validation/link-validator-evidence.png){fig-alt="A screenshot of the validation report section 2.1.1. that shows a compliance assessment with the option to link evidence" .screenshot}

5. On the **Link Validator Evidence to Validation Report** page that opens, select the evidence that is related to your assessment. 
   
   If you are not sure if something is relevant, you can expand the section by clicking **{{< fa chevron-down >}}** for more details.

6. Click **Update Linked Evidence**.

   - Confirm that the newly linked-to evidence shown under Developer Evidence is accurate.
   - Evidence that needs your attention gets flagged with [{{< fa triangle-exclamation >}} Requires Attention]{.bubble .yellow-bg}. These sections get flagged automatically whenever a test result is above or below a certain threshold.

## Link findings to reports

1. In the left sidebar, click **{{< fa cubes >}} Inventory**.

2. Select a model or find your model by applying a filter or searching for it.[^9]

3. In the left sidebar that appears for your model, click **{{< fa file >}} Documents** and select a Validation Report type file under Documents.[^10]

   Expand any subsection of the validation report you would like to work with.

   For example, locate 2. Validation then select **2.1.1. Assumptions**.

4. In any section of the report where the button is available, click **{{< fa link >}} Link Finding to Report**. 

   ![Validation report section 2.1.1. that shows a compliance assessment with the option to link to finding](link-finding.png){fig-alt="A screenshot of the validation report section 2.1.1. that shows a compliance assessment with the option to link to finding" .screenshot}

5. On the **Link Finding to Report** modal that opens, select from the list of available findings, or click **{{< fa plus >}} Create New Finding**.[^11]

6. Click **Update Linked Findings**.

   Confirm that newly linked finding shown under Findings is accurate.

## Provide compliance assessments

{{< include _provide-compliance-assessments.qmd >}}

## What's next

- [Collaborate with others](/guide/model-documentation/collaborate-with-others.qmd)
- [Working with model findings](working-with-model-findings.qmd)
- [Assign section statuses](/guide/model-documentation/assign-documentation-section-statuses.qmd)
- [View model activity](/guide/model-inventory/view-model-activity.qmd)


<!-- FOOTNOTES -->

[^1]: [Register models in the inventory](/guide/model-inventory/register-models-in-inventory.qmd)

[^2]: [Submit for approval](/guide/model-documentation/submit-for-approval.qmd)

[^3]: [Manage permissions](/guide/configuration/manage-permissions.qmd)

[^4]: [Working with the model inventory](/guide/model-inventory/working-with-model-inventory.qmd#search-filter-and-sort-models)

[^5]: [Working with model documents](/guide/templates/working-with-model-documents.qmd)

[^6]: [{{< var validmind.developer >}}](/developer/validmind-library.qmd)

[^7]: [Working with the model inventory](/guide/model-inventory/working-with-model-inventory.qmd#search-filter-and-sort-models)

[^8]: [Working with model documents](/guide/templates/working-with-model-documents.qmd)

[^9]: [Working with the model inventory](/guide/model-inventory/working-with-model-inventory.qmd#search-filter-and-sort-models)

[^10]: [Working with model documents](/guide/templates/working-with-model-documents.qmd)

[^11]: [Add and manage model findings](/guide/model-validation/add-manage-model-findings.qmd#add-findings-on-validation-reports)