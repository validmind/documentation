...
Add findings at the model or documentation section level, update your findings to include proposed remediation plans and supporting documentation, or delete findings that no longer need to be tracked.
...
- While finding deletion will be logged under your {{< fa wifi >}} Model Activity,[^11] deleted findings cannot be retrieved.
- Deleting a finding will also remove it from any validation reports[^12] it was previously attached to.

:::

If you logged a finding in error or otherwise no longer need to track that finding, you can delete it:

1. Locate the finding you want to delete.[^13]
...
[^10]: [Manage supporting documentation](#manage-supporting-documentation)

[^11]: [View model activity](/guide/model-inventory/view-model-activity.qmd)

[^12]: [Link findings to reports](/guide/model-validation/assess-compliance.qmd#link-findings-to-reports)

[^13]: [View and filter model findings](view-filter-model-findings.qmd)