:::: {.content-visible unless-format="revealjs"}

::: {.panel-tabset}

#### Add artifact fields

To add a new artifact field, click **{{< fa plus >}} Add Field**:

a. Artifact fields require a **[type]{.smallcaps}**,^[[Artifact field types](/guide/model-validation/manage-artifact-fields.qmd#artifact-field-types)] a **[name]{.smallcaps}**, and a **[artifact type]{.smallcaps}**.^[[Manage artifact types](/guide/model-validation/manage-artifact-types.qmd)]

    - (Optional) Enter a **[description]{.smallcaps}** to indicate the purpose of the field.
    - (Optional) You can also choose to indicate that a field is **[required when creating artifacts]{.smallcaps}**.
    - (Optional) You can also assign a field to a group.^[[Add artifact field groups](/guide/model-validation/manage-artifact-fields.qmd#add-artifact-field-groups)]
    - Assign **[write permissions]{.smallcaps}** to allow granular access to editing this field gated by model stakeholder types or user roles. Leave this field blank for default permissions where only the [{{< fa hand >}} Customer Admin]{.bubble} role has write access.[^edit-artifact-fields]

a. When you are satisfied with the setup of your artifact field, click **Save**.

#### Edit artifact fields

To edit an existing artifact field:

a. Click on the artifact field you would like to edit.

    Expand any inventory field group to see more fields.^[[Add artifact field groups](/guide/model-validation/manage-artifact-fields.qmd#add-artifact-field-groups)]

a. Make your desired changes to your artifact field, then click **Save**.

:::

<!-- ADDITIONAL FOOTNOTES -->

[^edit-artifact-fields]:

    ::: {.callout}
    ## All other users will have read-only access unless specified under the **[write permissions]{.smallcaps}**.

    - [**Model Stakeholders**](/guide/configuration/manage-model-stakeholder-types.qmd) — Write permissions on a per model basis.
    - [**Application Roles**](/guide/configuration/manage-roles.qmd) — Write permissions on a per field basis across all artifacts.

    :::

::::


:::: {.content-hidden unless-format="revealjs"}
1. In the left sidebar, click **{{< fa gear >}} Settings**.

1. Under Artifacts, select **Artifact Fields**. Here you can edit existing artifact fields, or add a new one:

    - Click **{{< fa plus >}} Add Field** to add a new field.
    - Click on an existing field to edit that field.

1. Artifact fields require a **[type]{.smallcaps}**, a **[name]{.smallcaps}**, and a **[artifact type]{.smallcaps}**.

    - (Optional) Enter a **[description]{.smallcaps}** to indicate the purpose of the field.
    - (Optional) You can also choose to indicate that a field is **[required when creating artifacts]{.smallcaps}**.
    - (Optional) You can also assign a field to a group.
    - Assign **[write permissions]{.smallcaps}** to allow granular access to editing this field gated by model stakeholder types or user roles.

1. When you are satisfied with the setup of your artifact field, click **Save**.

::::


