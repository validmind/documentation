...
- [x] Finding statuses have been set up for use with artifacts.[^3]
...
<!-- ADD FINDINGS VIA SECTION CURRENTLY DISABLED, DOCS REMOVAL REQUESTED BY ANDRES -->
...
4. In any section of the report where the button is available, click **{{< fa link >}} Link Finding to Report** and select the finding type[^8] you want to add:

   - Validation Issue
   - Policy Exception
   - Model Limitation

5. On the Link Finding to Report modal that opens, click **{{< fa plus >}} Add {x} Finding** where `{x}` is the type of finding.

6. On the Add Model Finding modal that opens, provide information for your finding.
...
[^9]: [View and filter model findings](view-filter-artifacts.qmd)
...
[^12]: [View and filter model findings](view-filter-artifacts.qmd)
