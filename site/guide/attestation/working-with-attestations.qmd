...
Attestation on the {{< var validmind.product >}} takes snapshots of models and their findings, ensuring that changes made after attestation creation do not affect the historical record.

The attestation process tracks attestation from assignment to final sign-off. Each attestation must pass several states, with responsibilities shared between the model owner and a validator:
...
- **In Progress** — While the model owner completes the questionnaire
- **Ready for Review** — When model owner submits the completed questionnaire
...
- **Signed Off** — Final state after a validator has signed off

Model owners and validators may exchange feedback during the review process represented by the dotted line in the diagram. 
