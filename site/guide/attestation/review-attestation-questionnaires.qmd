...
3. Optional: Click **{{< fa signal-stream >}} See Activity** to review:
...
5. Accept or reject the attestation questionnaire:
   ...
   ::: {.callout}
   **Next {{< fa arrow-right >}}** [Sign off on the attestation](#sign-off-attestations)
   ::: 

   - Click **Reject** to record the status change back to [**{{< fa clipboard-check >}} In progress**]{.bubble} in the [**notes**]{.small-caps} field, then click **Confirm**.

   ::: {.callout-important title="How to request changes"}
   ...
   ::: {.column-margin .callout title="Tip"}
   If you previously rejected an attestation questionnaire, click **{{< fa signal-stream >}} See Activity** to review the model owner’s response on resubmission. 
   :::
...
Following your [review of an attestation questionnaire](#review-attestation-questionnaires) or when prompted on your dashboard:
