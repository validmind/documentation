---
# Copyright © 2023-2026 ValidMind Inc. All rights reserved.
# Refer to the LICENSE file in the root of this repository for details.
# SPDX-License-Identifier: AGPL-3.0 AND ValidMind Commercial
title: "Manage text block library"
date: last-modified
---

{{< include /guide/templates/_text-blocks.qmd >}}

::: {.attn}

## Prerequisites

- [x] {{< var link.login >}}

:::

## Add text blocks

::: {.panel-tabset}

### Add new text blocks to library

{{< include /guide/templates/_add-text-blocks.qmd >}}

### Add existing text blocks to library

To transform an existing text block into a reusable block:

1. In the left sidebar, click **{{< fa cubes >}} Inventory**.

2. Select a record or find your record by applying a filter or searching for it.[^2]

3.  In the left sidebar that appears for your record, click **{{< fa file >}} Documents** and select the **Latest** tab.[^3]

4. Click on the file you'd like to copy a text block from.[^4]

5. Select the relevant section containing the text block you want to make reusable.

6. Hover your mouse over the text block you want to add to your block library, and click **{{< fa plus >}} Add to Library** to enter the details for your block:

    - The **Name** of your text block
    - (Optional) A **Description** for your text block
    - The templated **Content** for your text block
    - Whether the block **Sharing** permissions are **Private** or **Shared**

7. Click **Add Block** to save your text block.

Once added, insert private or shared text blocks into your documents,[^5] or shared text blocks into document templates.[^6]

:::

<!-- BC 2026 Apr 24: Seems like we removed the ability to reference field values in the form of variables in library text blocks, so hiding this info for now. -->

<!-- ::: {.panel-tabset}

#### Reference field values

While composing content for a library text block, you can reference values in the form of variables from inventory record fields. When the text block is inserted into a document, values from the associated record's inventory fields will automatically be populated in the content block if not empty.

{{< include /guide/documentation/content_blocks/_reference-field-values.qmd >}} -->

### Insert mathematical formulas

While composing content for a library text block, you can insert math equations using the formula editor:

{{< include /guide/documentation/content_blocks/_insert-mathematical-formulas.qmd >}}

<!-- ::: -->

4. Select a **Sharing** option:

    - **Private** — Visible only to you
    - **Shared** — Accessible across your organization

5. Click **Add Block** to save your text block.

## Duplicate text blocks

{{< include /guide/templates/_duplicate-text-blocks.qmd >}}

## Edit text blocks

::: {.callout title="Editing a text block within the block library will not update inserted instances of that block."}
Previously inserted instances of text blocks within documents will not be updated.
:::

To modify an existing text block in your block library:

1. In the left sidebar, click **{{< fa gear >}} Settings**.

2.  Under {{< fa file >}} Documents, select **Block Library**.

3. To the right of the text block you want to edit, click **{{< fa ellipsis-vertical >}}** and select **Edit Details**.

4. Make your desired changes to the following fields:

    - The **Name** of your text block
    - (Optional) A **Description** for your text block
    - The templated **Content** for your text block
    - Whether the block **Sharing** permissions are **Private** or **Shared**

5. Click **Save Block** to apply your changes.

## Delete text blocks

::: {.callout-important title="Text block deletion is permanent."}
While previously inserted instances of text blocks will not be affected, the text block will no longer be available for future use via the block library.
:::

To remove an existing text block from your block library:

1. In the left sidebar, click **{{< fa gear >}} Settings**.

2.  Under {{< fa file >}} Documents, select **Block Library**.

3. To the right of the text block you want to remove, click **{{< fa ellipsis-vertical >}}** and select **[{{< fa trash-can >}} Delete Text Block]{.red}**.

4. Click **Yes, Delete Block** to confirm permanent deletion of that block from your block library.


<!-- FOOTNOTES -->

[^1]: [Manage permissions](/guide/configuration/manage-permissions.qmd)

[^2]: [Working with the inventory](/guide/inventory/working-with-the-inventory.qmd#search-filter-and-sort-records)

[^3]: [Work with document versions](/guide/documentation/work-with-document-versions.qmd)

[^4]: [Manage documents](/guide/templates/manage-documents.qmd)

[^5]: [Work with content blocks](/guide/documentation/work-with-content-blocks.qmd#add-content-blocks)

[^6]: [Customize document templates](/guide/templates/customize-document-templates.qmd#add-text-blocks-to-templates)



