...
::: {.callout-important title="Workflows cannot be saved until condition branches are connected to other steps."}
You must also include a catch-all `else` branch path to ensure that your conditional logic is complete.
:::
...
For Status Change[^13], User Action[^14], and Condition Branch[^15] conditions, you're able to add a single independent **Rule** or a linked condition **Group**. These rules and groups can be nested if desired:
...
To initiate the beginning of your workflow:[^16]
...
1. First, configure an **Approval** step.[^17]

2. Then, drag two **Status Change** steps onto the canvas:[^18]
...
[^13]: [Status Change](#status-change) steps

[^14]: [User Action](#user-action) steps

[^15]: [Condition Branch](#condition-branch) steps

[^16]: After you’ve [configured your workflow steps](#configure-workflow-steps), you can then link your workflow together.
...
[^18]: [Status Change](#status-change) steps
