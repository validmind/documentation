...
This workflow is initiated manually,[^7] when validation is complete and the model is approved for implementation.

- After this workflow is manually started,[^8] the model owner schedules implementation by entering in a projected deployment date.
- The workflow will wait until the timestamp indicated in the scheduled deployment date before revealing the next available action in the workflow — in this case, the option to deploy the model and enter in a concrete date the model was pushed to production.
- Each action on this workflow is linked to a transition in model stage,[^9] as well as workflow state.[^10]

::: {.tc}
![Example model deployment cycle workflow](example_model-deployment-workflow.png){fig-alt="A screenshot showing an example model deployment cycle workflow" .screenshot group="model-validation"}
:::
...
![Example model stage change configuration modal](configure_model-stage-change.png){fig-alt="A screenshot showing an example model stage change configuration modal" .screenshot group="stage-change"}
:::

::: {.w-40-ns}

![Example model stage change step](example_model-stage-change.png){fig-alt="A screenshot showing an example model stage change step" .screenshot group="stage-change"}
...
![Example workflow state change configuration modal](configure_workflow-state-change.png){fig-alt="A screenshot showing an example workflow state change configuration modal" .screenshot group="state-change"}
:::

::: {.w-40-ns}

![Example workflow state change step](example_workflow-state-change.png){fig-alt="A screenshot showing an example workflow state change step" .screenshot group="state-change"}
...
[^7]: ![Adding a workflow that is manually initiated when a model is approved to be deployed](example_model-deployment.png){fig-alt="A screenshot showing the modal for addoing a workflow that is manually initiated when a model is approved to be deployed" .screenshot group="model-registration"}
...
[^8]: [Manage model workflows](manage-model-workflows.qmd#initiate-model-workflows)

[^9]: [Manage model stages](manage-model-stages.qmd)

[^10]: [Setting up workflows](setting-up-model-workflows.qmd#workflow-states)

[^11]: [Manage model stages](manage-model-stages.qmd)

[^12]: [Setting up workflows](setting-up-model-workflows.qmd#workflow-states)
