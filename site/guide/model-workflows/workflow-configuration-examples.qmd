...
![Example {{< fa arrow-pointer >}} User Action v2 button](example_user-action-button.png){fig-alt="A screenshot showing an example User Action v2 button" .screenshot group="user-action"}
...
In this example, the [peer reviewers]{.smallcaps} model stakeholders[^13] assigned to the model are presented with an approval message:
...
In this example, models where the field [genai model]{.smallcaps} is set to true undergo a special [legal review]{.smallcaps} model stakeholder[^14] approval process:[^15]
...
In this example, the next step on the workflow waits until the timestamp indicated in a [deployment scheduled]{.smallcaps} datetime field:[^16]
...
[^13]: [Manage model stakeholder types](/guide/configuration/manage-model-stakeholder-types.qmd)

[^14]: [Manage model stakeholder types](/guide/configuration/manage-model-stakeholder-types.qmd)

[^15]: [{{< fa users >}} Approval](#approval)

[^16]: [Manage model inventory fields](/guide/model-inventory/manage-model-inventory-fields.qmd)