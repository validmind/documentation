...
This workflow is initiated when a field on the model changes — in this case, when the model development team submits their model for validation and enters in a date in the [submitted for initial validation on]{.smallcaps} field via a step in the previous *On model registration* workflow.
