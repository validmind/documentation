...
This workflow is initiated when a field is populated — in this case, when the model is scheduled for deployment by entering value into the [deployment scheduled]{.smallcaps} datetime field.[^7]

- After this workflow is manually started, the model owner schedules implementation by entering in a projected deployment date.
...
- Each action on this workflow is linked to a transition in model stage,[^8] as well as workflow state.[^9]
