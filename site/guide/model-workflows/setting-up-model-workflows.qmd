...
| [request fields]{.smallcaps} (optional) | Include required inventory fields alongside your action.[^9]  |
...
| At Specified Date on Model Field | [model field]{.smallcaps}[^16] | Wait until a specific timestamp as defined by a model inventory field on a per model basis. If the selected field is empty[^17] or the milestone date inputted is in the past, the next workflow step will display immediately. |
...
- Select from either **User Roles**,[^20] **Model Stakeholder Types**,[^21] or either **User Role** or **User Type** inventory field[^22] for your **[approval group]{.smallcaps}**.
...
- **At Specified Date on Model Field** — Set the **[model field]{.smallcaps}**[^23] with a valid `Date` or `Date Time` type model inventory field that defines the specific timestamp on a per model basis.
...
Add workflow states by either while configuring a **{{< fa wifi >}} Workflow State Change** step,[^25] or via the **{{< fa gear >}} Settings** page:
...
[^11]: **[Configure custom workflows](configure-model-workflows.qmd#create-custom-workflows):**<br><br>
...
[^14]: [Configure custom workflows](configure-model-workflows.qmd#link-workflow-together)

[^15]: [Conditional requirements](#conditional-requirements)

<!-- HERE -->

[^16]: [Manage model inventory fields](/guide/model-inventory/manage-model-inventory-fields.qmd#inventory-field-types)

[^17]: Make model inventory fields **[required on registration]{.smallcaps}** to ensure necessary fields have valid values.

[^18]: [Configure custom workflows](configure-model-workflows.qmd#link-workflow-together)

[^19]: [Workflow step types](#workflow-step-types)

[^20]: [Manage roles](/guide/configuration/manage-roles.qmd)

[^21]: [Manage model stakeholder types](/guide/configuration/manage-model-stakeholder-types.qmd)

[^22]: [Manage model inventory fields](/guide/model-inventory/manage-model-inventory-fields.qmd#inventory-field-types)

[^23]: [Manage model inventory fields](/guide/model-inventory/manage-model-inventory-fields.qmd#inventory-field-types)

[^24]: [Manage model stages](manage-model-stages.qmd)

[^25]: [Configure custom workflows](configure-model-workflows.qmd#configure-workflow-steps)