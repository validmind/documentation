...
##### Link approval steps*
Approval steps need to be subsequently linked to both a [Rejected]{.bubble .red .red-bg} and an [Approved]{.bubble .green .green-bg} Model Stage Change step:

1. First, configure an **Approval** step.[^3]

2. Then, drag two **Model Stage Change** steps onto the canvas:[^4]
...
3. Connect the **Approval** step to the [Rejected]{.bubble .red .red-bg} and [Approved]{.bubble .green .green-bg} **Model Stage Change** steps with the red and green **{{< fa circle >}}** respectively by dragging from the colored **{{< fa circle >}}** to the top **{{< fa circle >}}** of the subsequent step.
...
2. Via a workflow applied to a model:
    ...
    b. Select a model or find your model by applying a filter or searching for it.[^5]
...
[^3]: [Setting up model workflows](/guide/model-workflows/setting-up-model-workflows.qmd#approval)

[^4]: [Setting up model workflows](/guide/model-workflows/setting-up-model-workflows.qmd#stage-change)
