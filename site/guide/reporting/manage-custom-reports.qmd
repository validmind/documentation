...
To work with custom reports, first create a custom analytics page, then add your custom visualizations: 
...
For example, to create a stacked bar chart grouped by business unit (x-axis) and workflow status (y-axis) for only models where the risk tier is equal to `Tier 1`, you would set:

- **[visualization type]{.smallcaps}** — `Bar Chart`
- **[dataset]{.smallcaps}** — `Inventory Models`
- **[metrics]{.smallcaps}** — `Model Name`, `Count` 
- **[grouping]{.smallcaps}** — `Business Unit Name`, `Workflow Status Name` 
- **[filtering]{.smallcaps}** — `Risk Tier Name` `is` `Tier 1`
...
::: {.w-50-ns}
![Setup for a stacked bar chart grouped by business unit (x-axis) and workflow status (y-axis) for only models where the risk tier is equal to `Tier 1`](custom-visualization-setup.png){fig-alt="Screenshot of the setup for a bar chart grouped by business unit (x-axis) and workflow status (y-axis) for only models where the risk tier is equal to `Tier 1`" .screenshot}

:::

::: {.w-40-ns}
![Workflow status (y-axis) on the custom visualization for only models where the risk tier is equal to `Tier 1`](example-custom-visualization.png){fig-alt="Screenshot of the workflow status (y-axis) on the custom visualization for only models where the risk tier is equal to `Tier 1`" .screenshot}
