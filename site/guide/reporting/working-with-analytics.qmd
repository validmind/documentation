...
- [x] There are validation issue type findings logged on any model.[^2]
...
: Count of your model volume by stage[^5] further broken down by business unit. Assists you in pinpointing where in the organization resources may need to be focused, based on the progress and severity of findings. 
...
: Count of your total validation issue type findings,[^6] currently open validation issues, currently past due validation issues, and previous validation issues now closed.
...
: Sorts validation issue type findings based on their resolution status, such as open, closed, or past due. Provides a snapshot of the validation process, highlighting which issues have been addressed, which are currently being worked on, and which are yet to be tackled.
...
: Specific to individual models, details the validation issue type findings for each model, including any errors, limitations, or assumptions that may affect the model's performance. This helps in assessing the reliability and accuracy of each model.
...
[^6]: [Manage finding types](/guide/model-validation/manage-finding-types.qmd#available-finding-types)
