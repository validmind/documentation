...
2. **Generate model documentation**: With the {{< var vm_framework >}}, generate automated model documentation and run validation tests. This step includes making use of the automation and testing functionality provided by the framework and uploading the output to the {{< var vm_ui >}}. You can iteratively regenerate the documentation as you work though the next step of refining your documentation.
...
: When the documentation template requires more tests to be added, or if the documentation template does not include a specific content or test block you need: 
   ...
   - For tests not provided by the framework: [Add your own external test provider](../notebooks/code_samples/custom_tests/integrate_external_test_providers.ipynb).
