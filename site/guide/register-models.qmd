---
title: "Register models in the inventory"
keywords: "register models, model inventory, ai risk, model risk management, ValidMind"
date: last-modified
---

Learn how to register a model you are documenting in the model inventory. This topic is relevant for model owners who want to enable their model development teams to use ValidMind's model documentation and validation features.  

## Prerequisites

* You are a registered user on the ValidMind Platform
* You are logged into the ValidMind Platform

## Steps

1. From the **Home** page, navigate to the **Model Inventory** page on the left. 
<!--- **Need screenshot** --->
   
2. In the **Model Inventory** page, click **Register new model**.
   
3. Fill in the required information on the registration form:

   - Provide a model name
   - Select the relevant business unit
   - Select the relevant model methodology being used
   - Select the relevant model use case
   - Provide a purpose statement to explain what the model will be used for
   - select the preliminary risk tier for the model

<!--- TO DO **Screenshot** --->
   
4. Click **Register new model** to create a new entry in the model inventory.

   You can now access the model details from the **Model Inventory** page.

<!---
## Troubleshooting

[Include any common issues or errors that may arise during the task and how to resolve them.]
--->

## Related topics

* [Edit model inventory fields](edit-model-inventory-fields.qmd)
* [Create a new documentation project](create-documentation-project.qmd)
