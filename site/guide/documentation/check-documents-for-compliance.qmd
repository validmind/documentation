...
Each check is saved automatically as a run, including one you stop early. Runs are private by default — only you can see your own runs until you publish them to your organization. Published runs are visible to everyone in your organization.
...
### Stop a check that is running

A check that is still running can be stopped from **{{< fa inbox >}} My Inbox**, from its progress notification, or from the {{< var vm.checker >}} itself. You are asked to confirm first, and you choose whether to keep what it produced:

- **Keep the run** — the answers already completed are saved. Questions the check never reached are marked as not checked, and the question count reflects what was actually assessed.
- **Discard the run** — the run and its answers are deleted.

A stopped run is labelled as cancelled wherever it appears, so a partial check is never mistaken for a finished one:

- It **cannot be published** to your organization. Only a completed check can be shared.
- Exporting it produces a report that states the check did not finish.

To run the full check, start a new one. A stopped run is kept as history and is not resumed.
