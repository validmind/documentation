...
Status Change
: Transitions a status. Requires [available resource statuses](customize-resource-statuses.qmd) associated with the workflow.
...
Approval
: When your resource reaches the approval step, users in the selected Approval Group will receive your configured message. Requires a User type [custom field](manage-inventory-custom-fields.qmd) to select from, and must be connected to both an approval and rejection Status Change step.
...
::: callout
**Example:** A message shown to the Approval Group users requesting approval of model documentation.
:::
...
1. Drag an **Approval** step to the canvas:

    - Select the User type custom field in Approval Group.
    - Determine an approval Threshold.
    - Enter an Approval Message with a Title and Message content.

2. Drag two Status Change steps onto the canvas.

    - Assign a Rejected status to one.
    - Assign an Approved status to the other.
    - Connect the Approval step to the Rejected and Approved Status Change steps with the red and green **{{< fa circle >}}** respectively.
