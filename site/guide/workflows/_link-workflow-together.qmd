...
{{< fa users >}} Approval steps need to be subsequently linked to both a [Rejected]{.bubble .red .red-bg} and an [Approved]{.bubble .green .green-bg} {{< fa cube >}} Inventory Record Stage Change step:^[[Workflow step types](/guide/workflows/workflow-step-types.qmd)]

i. First, configure an **{{< fa users >}} Approval** step.

ii. Then, drag two **{{< fa cube >}} Inventory Record Stage Change** steps onto the canvas:

    - Assign a stage associated with rejection to one in the **[set inventory record stage to]{.smallcaps}** field.
    - Assign a stage associated with approval to the other in the **[set inventory record stage to]{.smallcaps}** field.
