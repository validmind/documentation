...
| Field | Description |
|---:|---|
| Target Field | Select a field to update with a given value,^[From the **{field name}** drop-down, where `{field name}` is the name of the field you want to update, select the value you want to automatically set.], check **Clear field (set to empty)** to remove the existing field value, or **Generate with AI** to enter a prompt to guide automatic population of an LLM-suggested value.[^set-field-ai]  |
...
[^set-field-ai]:
    ...
    ValidMind will generate a field value using your prompt, and associated record or artifact data using AI:
    <br><br>
    - Write a clear instruction, starting with an imperative verb. For example: `Summarize`, `Draft`, `Extract`
    - Use **Insert Variable** to dynamically reference record or artifact fields and details.
