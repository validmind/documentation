...
| **Target Inventory Record Type** | Choose **A fixed type**, or **From a field value** so the target is read from a record field at runtime (the field value should be a record-type slug such as `agent`, or the type id). |
...
Typical **Manual review** pattern: earlier in the intake workflow, a reviewer chooses the target type (for example with a **{{< fa arrow-pointer >}} User Action** that sets a field). Configure this step with **Manual review** and, when needed, **From a field value** so the step applies that selection and labels it as a manual-review classification.
:::
