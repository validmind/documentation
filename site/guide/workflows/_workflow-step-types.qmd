...
When a workflow reaches a webhook step, the workflow is paused. Set up a webhook step to listen for a `POST` request that will resume the workflow:
...
1. Click **{{< fa copy >}}** to the right of the the `x-api-key` and its value to copy them to your clipboard.
2. Click **{{< fa copy >}}** to the right of the the `x-api-secret` and its value to copy them to your clipboard.
3. Include these headers with your `POST` request to authenticate your call.
