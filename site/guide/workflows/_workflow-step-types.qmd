...
| **Title** (required) | The artifact's title.[^artifact-text] |
...
[^broadcast]: Use the rich text editor to compose your **Subject** and **Message**:
    ...
    2. In a workflow started by a monitoring threshold breach, a **Monitoring Breach** group also offers the metric that breached, its value, and its thresholds — see [Trigger workflows on threshold breach](/guide/monitoring/trigger-workflows-on-threshold-breach.qmd#include-breach-details-in-an-artifact-or-a-notification).
    3. Click on the **Preview** tab and choose a record or artifact under **Select {inventory record/artifact} for Preview** to confirm how the notification will appear when rendered.

[^artifact-text]: Use the rich text editor to compose your **Title** and **Description**:

    1. Include optional details about the record the workflow is running on using the **{ } Insert Variable** drop-down.
    2. In a workflow started by a monitoring threshold breach, a **Monitoring Breach** group also offers the metric that breached, its value, and its thresholds — see [Trigger workflows on threshold breach](/guide/monitoring/trigger-workflows-on-threshold-breach.qmd#include-breach-details-in-an-artifact-or-a-notification).
    3. Variables that cannot be filled in for this step are not offered. There is no recipient here, so no recipient variable is available, and the artifact does not exist yet when its text is composed. Link variables are not offered either: a link is written differently for each reader, and this step's text is stored once for everyone to read.

    **Title** is stored as plain text, so any formatting you apply to it is discarded. **Description** keeps its formatting, as it does everywhere else an artifact's description is edited.
