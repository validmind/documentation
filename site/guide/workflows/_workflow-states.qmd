:::: {.content-visible unless-format="revealjs"}
Workflow states are statuses unique to a specific workflow, discrete from model stages.^[[Manage model stages](/guide/workflows/manage-model-stages.qmd)]

#### Add workflow states

Add workflow states by either while configuring a **{{< fa wifi >}} Workflow State Change** step,^[[Configure workflow steps](/guide/workflows/configure-workflows.qmd#configure-workflow-steps)] or via the **{{< fa gear >}} Settings** page:

1. In the left sidebar, click **{{< fa gear >}} Settings**.

1. Under Workspace, select **Workflow States**.

1. Select the tab for the type of state to add:

    - **Model Workflows** — Workflow states that apply to model workflows.
    - **Finding Workflows** — Workflow states that apply to finding workflows.

1. Click on the workflow you'd like to modify workflow states for.

1. Click **{{< fa gear >}} Add Workflow State**.

1. Enter in a **[name]{.smallcaps}** and a **[description]{.smallcaps}**, then select a **[color]{.smallcaps}** for your workflow state.

1. Click **Save** to create the workflow state.

#### Edit workflow states

1. In the left sidebar, click **{{< fa gear >}} Settings**.

1. Under Workspace, select **Workflow States**.

1. Select the tab for the type of state you want to edit:

    - **Model Workflows** — Workflow states that apply to model workflows.
    - **Finding Workflows** — Workflow states that apply to finding workflows.

1. Click on the workflow you'd like to edit the states for.

1. Hover over the workflow state until **{{< fa pencil >}} Edit Details** appears, then click on it.

1. Make your changes to the **[name]{.smallcaps}**, **[description]{.smallcaps}**, and **[color]{.smallcaps}** for your workflow state.

1. Click **Save** to apply your changes.

#### Delete workflow states

::: {.callout-important title="Workflow state deletion is permanent."}
If a state is or was previously in use  on a workflow within a {{< fa wifi >}} Workflow State Change step, the state cannot be deleted.
:::

1. In the left sidebar, click **{{< fa gear >}} Settings**.

1. Under Workspace, select **Workflow States**.

1. Select the tab for the type of state you want to delete:

    - **Model Workflows** — Workflow states that apply to model workflows.
    - **Finding Workflows** — Workflow states that apply to finding workflows.

1. Click on the workflow you'd like to delete workflow states for.

1. Hover over the workflow state until **{{< fa trash-can >}} Delete Workflow State** appears, then click on it.

1. After you confirm, the workflow state will be removed.

::::


:::: {.content-hidden unless-format="revealjs"}
Workflow states are statuses unique to a specific workflow, discrete from model stages.

Add workflow states by either while configuring a **{{< fa wifi >}} Workflow State Change** step, or [via the **{{< fa gear >}} Settings** page](/guide/workflows/setting-up-workflows.qmd#workflow-states){target="blank"}.

::::