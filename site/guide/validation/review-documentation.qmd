---
# Copyright © 2023-2026 ValidMind Inc. All rights reserved.
# Refer to the LICENSE file in the root of this repository for details.
# SPDX-License-Identifier: AGPL-3.0 AND ValidMind Commercial
title: "Review documentation"
date: last-modified
aliases:
  - /guide/review-model-documentation.html
  - /guide/model-validation/review-model-documentation.html
---

Use the {{< var validmind.platform >}} to review the documentation provided by developers as part of the formal validation process.

::: {.attn}

## Prerequisites

- [x] {{< var link.login >}}
- [x] A developer has submitted documentation for review.[^1]
- [x] You are a [{{< fa circle-check >}} Validator]{.bubble} or assigned another role with sufficient permissions to perform the tasks in this guide.[^2]

:::

## Key concepts

{{< include /about/glossary/validation_reports/_review.qmd >}}

## Document Overview

{{< include /guide/documentation/_document-overview.qmd >}}

{{< include /guide/documentation/_locate-document-overview.qmd >}}

::: {.panel-tabset}

#### Unresolved conversations
Hover over **{{< fa comment-dots >}} #** to see a preview of the unresolved conversation associated with the documentation section.

Resolve conversations directly from this popup, or click to expand the conversation.

#### Validation issues
Hover over **{{< fa triangle-exclamation >}} #** to see a preview of validation issues associated with the documentation section.

Click to expand the artifact.

#### Completion status
The **{{< fa check >}} #/# Done** indicator reflects the completion status of your documentation sections.

:::

### Recent activity
At the bottom of the Document Overview page, you will also see quick view of any recent documentation activity.

- Use the top tabs to narrow down the recent activity.
- Click on **{{< fa eye >}} View All Activity** to be taken to the **{{< fa wifi >}} Activity** page.



<!-- FOOTNOTES -->

[^1]: [Submit for approval](/guide/documentation/submit-documents.qmd)

[^2]: [Manage permissions](/guide/configuration/manage-permissions.qmd)