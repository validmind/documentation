...
Configure scheduled exports of platform datasets to cloud storage so that BI tools such as Tableau, Snowflake, and Looker can consume models, findings, workflow metrics, and activity events.
...
4. From the **[datasets]{.smallcaps}** dropdown, choose which datasets to export. For example:
   ...
   ::: {.callout title="Activity exports"}
   Model Activity exports audit events — comments, workflow status changes, and field updates on inventory records. Settings Activity exports organization-level settings and configuration changes as CSV files. Use these datasets for compliance reporting or audit trails.
   :::

   ::: {.callout title="Rolling activity windows"}
   Activity exports use a rolling window of **Last 30 days**, **Last 60 days**, or **Last 90 days**. A rolling window is required for Settings Activity so scheduled exports do not export the full audit history on each run.
   :::
...
::: {.column-margin}
![Selecting datasets](configure-analytics-export-step-1.png){fig-alt="Create Data Export modal showing Select Datasets step and datasets dropdown." .screenshot}
:::
...
::: {.column-margin}
![Configuring your cloud storage destination](configure-analytics-export-step-2.png){fig-alt="Create Data Export modal showing Configure Destination step with cloud storage and output settings." .screenshot}
:::
...
::: {.column-margin}
![Setting the schedule](configure-analytics-export-step-3.png){fig-alt="Create Data Export modal showing Set Schedule step with Daily schedule and 12:00 am UTC." .screenshot}
:::

9. Click **Create Data Export**.
...
## Disable or enable data exports
...
::: {.callout-warning title="Deleting a data export configuration is permanent."}
...
[^5]: [View settings activity](/guide/reporting/view-settings-activity.qmd)
