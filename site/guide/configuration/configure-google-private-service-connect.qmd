...
1. Create a private subnet in a supported GCP region with at least a `/28` of usable IP address space. 

     This subnet will be used to expose two ValidMind services.

2. Optional: Consider enabling Private Google Access to enable access to enable other services.
<!-- TO DO Link to Google docs for this -->

3. Pre-provision two IP addresses in this subnet for later use. 

## Create an endpoint to connect to ValidMind

Set up a Private Service Connect endpoint named `private` for accessing the ValidMind service securely and privately, with service discovery managed via Google Cloud’s Service Directory:
...
2. Create a new Private Service Connect endpoint:
   ...
   <!-- TO DO What does enabling entail? The screenshot doesn't show (There's a checkbox) -->
