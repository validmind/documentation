...
PrivateLink works by creating a private _VPC endpoint_ for a supported AWS service within your virtual private cloud. This endpoint acts as a proxy between your VPC and {{< var vm.product >}}, allowing traffic to be routed privately over the AWS network. To make the endpoint easier to use, {{< var vm.product >}} provides a _private DNS name_ that developers and validators can connect to in a browser.
...
2. Contact {{< var vm.product >}} at [{{< var support.email >}}](mailto:{{< var support.email >}}?subject=VPC%20Endpoint%20Connection%20Request) to get your new VPC endpoint connection request accepted. Include the following information:
