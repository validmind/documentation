...
::: {.callout title="This approach simplifies user management by centralizing permissions and enhances security and efficiency."}
It ensures that your users have the appropriate level of access based on their roles, reducing the risk of unauthorized actions and fostering a structured and controlled environment within an organization's digital ecosystem.
:::
...
On the left, the possible CRUD (`create`, `read`, `update`, `delete`) operations are listed. 

There are four [supported user roles](#supported-roles), along with a number of users that have been defined. Each user is a member of one or more groups where they can perform actions based on the roles that they have been given.
