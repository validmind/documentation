...
As monitoring progresses, it's vital to report each model's status to the model risk management committee. Regular updates with summary metrics should keep the committee informed of findings and emerging risks, highlighting significant trends or issues that may need action.
...
The implementation of the ongoing monitoring plan typically also falls on model developers. This effort includes executing the monitoring activities, collecting performance data, and generating reports. The first line also ensures that the monitoring is carried out according to the established schedule and that any anomalies or deviations are promptly identified and addressed.

### Testing

Model monitoring should incorporate a variety of tests to ensure ongoing accuracy and reliability. Key tests to pay attention to:
...
3. [Access monitoring results](#access-monitoring-results)
...
To enable ongoing monitoring for a model, you need to modify this code snippet to add `monitoring=True`. 
...
Before you can start sending ongoing monitoring data from the {{< var validmind.developer >}}, you must also select a monitoring template:
...
Ongoing monitoring results for your model are uploaded to the {{< var vm.platform >}}:
