...
| Monitoring Document URL | A link to the record's monitoring document. Available to a [{{< fa bullhorn >}} Broadcast]{.bubble} step only. |
: **Monitoring Breach** variables {.hover tbl-colwidths="[35,65]"}

A link is written differently for each person who reads it, so the two link variables — **Monitoring Document URL** here and **Record URL** under **Record Details** — are offered to a [{{< fa bullhorn >}} Broadcast]{.bubble} step, which writes its text once per recipient, and not to a [{{< fa expand >}} Create Artifact]{.bubble} step, which writes once and stores the result for everyone.
