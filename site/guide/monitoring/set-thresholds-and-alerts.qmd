...
def passed_fn(value):
    return value > thresholds["high_risk"]
...
- The custom function evaluates if `0.75 > 0.5` (high risk threshold), returning `True`.
- This evaluation results in `passed=True`, displaying a [{{< fa check >}} Satisfactory]{.bubble .green-bg} status badge.
- While the metric passes the custom function check, it would still trigger the medium risk threshold since `0.75` is above `0.6`.
...
::: {.w-50-ns .pr2}
If a logged metric breaches a threshold, alert notifications are triggered. 
