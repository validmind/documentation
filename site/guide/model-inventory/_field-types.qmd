...
Aggregation
: Automatically compute a read-only value from downstream artifacts^[[Working with artifacts](/guide/model-validation/working-with-artifacts.qmd)] linked to this model.

1. Select the type of **[artifacts]{.smallcaps}** to aggregate from.^[[Manage artifact types](/guide/model-validation/manage-artifact-types.qmd)]
...
2. Select an **[aggregation function]{.smallcaps}**:^[For MIN or MAX, also select the Date type **[artifact field]{.smallcaps}** to aggregate on.]
    ...
    - **MIN - Earliest/minimum value** — Return the least recent value from a selected Date type artifact fiel for that type.
...
: Automatically compute a read-only value from [downstream artifacts](/guide/model-validation/working-with-artifacts.qmd){target="_blank"} linked to this model.
    ...
    Aggregation is available on model inventory fields only.
