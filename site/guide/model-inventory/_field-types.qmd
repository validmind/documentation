...
### Example — Date and time calculations

Formulas can use date and time field types for scheduling and countdowns. The following field types are available in your formula:
...
Create these fields first so they appear in the **[available model fields]{.smallcaps}** dropdown:

- **Approved Date** — Date field for when the model was approved,
- **Risk Tier** — Single-select field with options, for example `Tier 1`, `Tier 2`, and `Tier 3`.

##### Example formula

Add an interval to an approval date based on risk tier:

```python
def formula(params):
    # Get approval date (stored as milliseconds timestamp)
    approved_date = getattr(params, "dmApprovedDate", "")
    if not approved_date:
        return "Not applicable"
    approved_date = date.fromtimestamp(int(approved_date) / 1000)
    
    # Define review intervals based on risk tier
    review_frequency = {
    ...
    # Look up frequency for this model's risk tier
    frequency = review_frequency.get(getattr(params, "dmRiskTier", ""))
    if not frequency:
        return "Not applicable"
    
    # Calculate and return next review date
...
Create this field first so it appears in the **[available model fields]{.smallcaps}** dropdown:

- **Next Review Date** — The Calculation field from the previous example or another Date field

##### Example formula

Use `vm_today` to show a countdown to the next review (formula re-evaluates so the countdown updates over time):
...
Date fields are passed into formulas as millisecond timestamps; use `date.fromtimestamp(int(params.fieldName) / 1000)` to convert. Formula results are strings — use `.isoformat()` for date values.
