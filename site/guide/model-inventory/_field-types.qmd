...
: Define a `formula(params)` function that automatically calculates and returns a value based on the params dictionary, which includes selected custom field keys retrieved from your other inventory model fields.[^calculation-field-restrictions]
    ...
    4. Click **Test Calculation {{< fa caret-down >}}** to open the testing area.
    ...
    For example, say you have numeric model inventory fields of `materiality` and `complexity`, where a larger value indicates a lower risk:[^risk-tier-calculation]
...
[^calculation-field-restrictions]:
