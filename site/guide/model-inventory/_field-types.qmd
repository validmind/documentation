...
Formulas can use date and time field types for scheduling and countdowns which can be used to calculate the next review date and the days remaining until the next review.

The following field types are available in formulas:
...
This example shows how to uses calculation fields to determine the next review date based on an approval date and the frequency of validation.
...
4. Create a `Next Review Date` calculation field that depends approved date and risk tier
...
6. Make a workflow depend on `Approved Date`.

7. Change the `Approved Date` after the fact and see how `Next Review Date` changes.

#### Days remaining

1. Create an `Approved Date` date field.

2. Create a `Next Review Date` field that depends approved date + some time delta:
   ...
   def formula(params):
       ...
       # Calculate the next review date
       next_review_date = approved_date + relativedelta(months=3)
       return next_review_date.isoformat()
   ```
