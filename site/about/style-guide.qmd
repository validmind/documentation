...
- **Bolding:** Some light bolding can be helpful to draw attention to core concepts. 
...
### Product names
Within [our documentation](https://docs.validmind.ai/), you are able to reference the {{<var vm_framework>}} and {{<var vm_platform>}} [via variables](https://quarto.org/docs/authoring/variables.html#var). 

- Use the variables shown on the table below instead of writing out the phrases to enable consistency between guides. 
...
| Product Name | Variable Key |
|------:|-----|
| {{<var vm_framework>}} |  `{{{<var vm_framework>}}}` |
| {{<var vm_platform>}} |  `{{{<var vm_platform>}}}` |
| {{<var vm_dev>}} |  `{{{<var vm_dev>}}}` |
| {{<var vm_ui>}} |  `{{{<var vm_ui>}}}` |
: ValidMind product variable keys {.hover}

> See also **[the ValidMind Glossary](glossary.qmd)** for extended information on product names. 

# Other resources
::: {.callout-tip title="Become a better writer" appearance="minimal"}
