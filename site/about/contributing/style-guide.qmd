...
::: {.column-margin}
^1^ Microsoft's guide on [bias-free communication](https://learn.microsoft.com/en-us/style-guide/bias-free-communication).
:::
...
- **Show only what’s necessary when necessary.** Progressive disclosure^2^ can help guide workflows as well as minimize visual clutter. 

::: {.column-margin}
^2^ Wikipedia article on [progressive disclosure](https://en.wikipedia.org/wiki/Progressive_disclosure).
:::

## American English
While the ValidMind community spans far and wide, its heart finds its home in Palo Alto, California. When writing for ValidMind, keep things consistent by using American English^3^ spelling and grammar conventions. 
...
::: {.column-margin}
^3^ United States Department of State [resources on American English](https://americanenglish.state.gov/).
:::
...
- Reference titles should include only the name of the content described unless it's the top-level topic, in which case the title should end in "reference".
...
::: {.column-margin}
^4^ Microsoft's guide on [sentence-style capitalization](https://learn.microsoft.com/en-us/style-guide/capitalization). 
:::
...
::: {.column-margin}
^5^ Quarto's guide on [callout blocks](https://quarto.org/docs/authoring/callouts.html).
:::
...
- When possible, instead of multiple inline links, make use of Quarto's margin content^6^ class.
...
::: {.column-margin}
^6^ Quarto's guide on [margin content](https://quarto.org/docs/authoring/article-layout.html#margin-content).
:::
...
^14^ [Code samples](../guide/samples-jupyter-notebooks.html)
:::
...
::: {.callout}
Refer also to [the glossary](../glossary/glossary.qmd) for extended information on product names. 
:::
