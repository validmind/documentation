...
#### Filenames
Filenames should generally match the title of the article[^8] or concisely summarize the content, and be descriptive but not overly lengthy. 
...
#### Mathematics
- Mathematical formulas should be rendered using LaTeX formatting.[^8] 
...
## Product names
Within our documentation (`https://docs.validmind.ai/`), you are able to reference constants such as the {{< var validmind.developer >}} and {{< var validmind.platform >}} via variables.[^13]
...
- Please note that variables will not work within any of the Jupyter Notebook code samples[^16] as these are technically standalone files.
...
- It makes use of Tachyons CSS[^19] to provide demostration overlays:
...
[^13]: **Quarto:** [Variables](https://quarto.org/docs/authoring/variables.html#var)
...
[^16]: [Code samples](/developer/samples-jupyter-notebooks.qmd)
