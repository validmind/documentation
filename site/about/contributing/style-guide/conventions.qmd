...
- By default, lightbox style images to allow users to enlarge for details is set globally.[^11] <!-- THIS DOESN'T SEEM TO WORK ATM  --> <!-- The `.lightbox` class can be disabled on a per-image basis by appending an image with `.no-lightbox`.  -->
