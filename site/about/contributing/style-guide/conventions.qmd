...
#### Callouts
We use two types of callouts[^6] to call attention to supplementary information or issue warnings:
...
#### Filenames
Filenames should generally match the title of the article[^7] or concisely summarize the content, and be descriptive but not overly lengthy. 
...
#### Code
- Format code in its own code block.[^8]
- Declare the code language within the pre-formatted block to properly trigger syntax highlighting.

Within a Jupyter Notebook, simply use a **code cell**[^9] rather than a markdown cell.
...
#### Mathematics
- Mathematical formulas should be rendered using LaTeX formatting.[^10] 
- On our WordPress blog posts, this is taken care of by the WP Quick LaTeX plugin.[^11]
...
## Product names
Within our documentation (`https://docs.validmind.ai/`), you are able to reference constants such as the {{< var validmind.developer >}} and {{< var validmind.platform >}} via variables.[^12]

- Use the variables shown on the table below instead of writing out the phrases to enable consistency between guides everywhere except for image alt text or Mermaid charts.[^13]
- If product names need to be updated, simply amend the `_variables.yml` file[^14] to see changes reflected throughout all guides. 
- Please note that variables will not work within any of the Jupyter Notebook code samples[^15] as these are technically standalone files.
...
- It makes use of Tachyons CSS[^16] to provide demostration overlays:
...
- It uses inline links only instead of footnotes,[^17] as footnotes are not visible in presentation mode.
...
[^6]: **Quarto:** [Callout Blocks](https://quarto.org/docs/authoring/callouts.html)

[^7]: [Titles](#titles)

[^8]:  **Quarto:** [Source Code](https://quarto.org/docs/authoring/markdown-basics.html#source-code)

[^9]: **Jupyter Notebooks:** [Code Cells](https://jupyter-notebook.readthedocs.io/en/stable/notebook.html#code-cells)

[^10]: **Quarto:** [LaTeX Equations](https://quarto.org/docs/output-formats/html-basics.html#latex-equations)

[^11]: **WordPress:** [WP QuickLaTeX](https://en-gb.wordpress.org/plugins/wp-quicklatex/)

[^12]: **Quarto:** [Variables](https://quarto.org/docs/authoring/variables.html#var)

[^13]: [Mermaid charts](https://mermaid.js.org/)

[^14]: [`_variables.yml`](https://github.com/validmind/documentation/blob/main/site/_variables.yml) file

[^15]: [Code samples](/developer/samples-jupyter-notebooks.qmd)

[^16]: **GitHub:** [Tachyons Extension For Quarto](https://github.com/nareal/tachyons)

[^17]: [Inline links](#inline-links)