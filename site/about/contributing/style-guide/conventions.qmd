...
- By default, lightbox style images to allow users to enlarge for details is set globally. 

:::: {.flex .flex-wrap .justify-around}

::: {.w-40-ns}
The `.lightbox` class can also be disabled at the page level for all images by adding the following to the YAML header:
...
::: {.w-40-ns}
::: {.callout}
On pages where you only want certain images to be enlargeable, set the page-level lightbox directive to `false` then append `.lightbox` to the images that are an exception to this rule.
:::
...
- Training materials use several supplementary style sheets to apply an alternate site theme, applied by a directory metadata file:[^24] 

  a. **{{< var validmind.training >}} pages:** `training.css`[^25]
  b. **Course slides:** `slides.scss`[^26]
...
- To use lightbox style images within training slides,[^30] add the following to the YAML header:
...
[^24]: **Quarto:** [Directory Metadata](https://quarto.org/docs/projects/quarto-projects.html#directory-metadata)

[^25]: **{{< var vm.product >}} GitHub:** [training.css](https://github.com/validmind/documentation/blob/main/site/training/assets/training.css)
