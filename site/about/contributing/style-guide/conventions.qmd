...
|  In the left sidebar, click **{{< fa cubes >}} Model Inventory**.  |  In the left sidebar, click **{{< fa cubes >}} model inventory**.  |
...
- In single-source files, either place the link inline for training materials formatted in `revealjs`,[^6] or use an embedded footnote for our normal user guides. 
  ...
  |------|-----|
  | ![Example of inline links in training materials](training_inline-links.png){fig-alt="A screenshot showing an example of inline links in training materials" .screenshot group="links" .lightbox} | ![Example of margin footnotes in a user guide](user-guide_footenotes.png){fig-alt="A screenshot showing an example of margin footnotes in a user guide" .screenshot group="links" .lightbox} |
...
| Correct | Incorrect | 
|------|-----|
| ![Correct example of footnote links automatically placed in the margin](margin-footnotes_correct.png){fig-alt="A screenshot showing a correct example of footnote links automatically placed in the margin" .screenshot group="footnotes" .lightbox} | ![Incorrect example of supplementary links in inline instructional text](inline-links_incorrect.png){fig-alt="A screenshot showing an incorrect example of supplementary links in inline instructional text" .screenshot group="footnotes" .lightbox} |
...
|   In the {{< var validmind.platform >}}, click **{{< fa cubes >}} Model Inventory** on the left sidebar.  | In the {{< var validmind.platform >}}, click "{{< fa cubes >}} Model Inventory" on the left sidebar. |
...
- By default, lightbox style images to allow users to enlarge for details is set globally.[^11] The `.lightbox` class can be disabled on a per-image basis by appending an image with `.no-lightbox`. 
...
- Registration pages make use of our custom `.preview` extension[^29] to display the tile card for the course. These previews act like links but they provide a live preview of the page linked to. 
...
- Training slides make use of Tachyons CSS[^30] styled with our custom `.overlay` class to provide demonstration overlays.
...
[^21]: [{{< var validmind.training >}}](/training/training.qmd)

[^22]: **Quarto:** [Revealjs](https://quarto.org/docs/presentations/revealjs/)

[^23]: [Course registration](#course-registration)

[^24]: [Course slides](#course-slides)

[^25]: **Quarto:** [Directory Metadata](https://quarto.org/docs/projects/quarto-projects.html#directory-metadata)

[^26]: **{{< var vm.product >}} GitHub:** [training.css](https://github.com/validmind/documentation/blob/main/site/training/assets/training.css)

[^27]: **{{< var vm.product >}} GitHub:** [slides.scss](https://github.com/validmind/documentation/blob/main/site/training/assets/slides.scss)

[^28]: **{{< var vm.product >}} GitHub:** [preview](https://github.com/validmind/documentation/tree/main/site/_extensions/nrichers/preview) extension
