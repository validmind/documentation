...
- When possible — such as any time instructions are not single-sourced — instead of multiple inline links, make use of Quarto's footnote functionality.
- Number all footnotes, and place the footnotes at the very end of the page and they will automatically appear in the correct location in the margin:
    ...
    [^4]: <!-- INDENT FOR BULLETS & MULTIPLE LINES -->
...
| Correct | Incorrect | 
|------|-----|
| ![](margin-footnotes_correct.png){fig-alt="A screenshot showing a correct example of supplementary links in margin content."} | ![](inline-links_incorrect.png){fig-alt="A screenshot showing an incorrect example of supplementary links in inline instructional text."} |
...
#### Filenames
Filenames should generally match the title of the article[^5] or concisely summarize the content, and be descriptive but not overly lengthy. 
...
#### Code
- Format code in its own code block[^6].
- Declare the code language within the pre-formatted block to properly trigger syntax highlighting.

Within a Jupyter Notebook, simply use a **code cell**[^7] rather than a markdown cell.
...
#### Mathematics
Mathematical formulas should be rendered using LaTeX formatting[^8]. On our WordPress blog posts, this is taken care of by the WP Quick LaTeX plugin[^9]. 
...
### Product names
Within our documentation (`https://docs.validmind.ai/`), you are able to reference constants such as the {{< var vm_framework >}} and {{< var vm_platform >}} via variables[^10]. 

- Use the variables shown on the table below instead of writing out the phrases to enable consistency between guides everywhere except for image alt text or Mermaid charts[^11].
- If product names need to be updated, simply amend the `_variables.yml` file[^12] to see changes reflected throughout all guides. 
- Please note that variables will not work within any of the Jupyter Notebook code samples[^13] as these are technically standalone files.
...
[^6]:  Quarto: [Source Code](https://quarto.org/docs/authoring/markdown-basics.html#source-code)

[^7]: Jupyter Notebooks: [Code Cells](https://jupyter-notebook.readthedocs.io/en/stable/notebook.html#code-cells)

[^8]: Quarto: [LaTeX Equations](https://quarto.org/docs/output-formats/html-basics.html#latex-equations)

[^9]: WordPress: [WP QuickLaTeX](https://en-gb.wordpress.org/plugins/wp-quicklatex/)

[^10]: Quarto: [Variables](https://quarto.org/docs/authoring/variables.html#var)

[^11]: [Mermaid charts](https://mermaid.js.org/)

[^12]: [`_variables.yml`](https://github.com/validmind/documentation/blob/main/site/_variables.yml) file

[^13]: [Code samples](/developer/samples-jupyter-notebooks.qmd)