...
- **Multi-tenant cloud** — Multiple organizations (tenants) share infrastructure while keeping data isolated, providing cost-efficiency and scalability. VPC support allows secure network connections.

- **Virtual Private ValidMind (VPV)** — A single-tenant architecture where one organization uses a dedicated infrastructure instance, offering greater control, customization, and enhanced security. This option is ideal for high-compliance needs and also supports VPC for secure connectivity.

## Architecture overview

Model developers run models using tools like Python, Jupyter, and R, accessing data from sources such as Google Cloud Storage, Amazon S3, and Snowflake.
...
These models are integrated with the {{< var validmind.developer >}}, which communicates with {{< var vm.product >}} via our API"

The ValidMind Platform includes:
...
Deployments support secure access via **virtual private cloud (VPC)**, ensuring privacy and control over resources. VPC integration is available for:
