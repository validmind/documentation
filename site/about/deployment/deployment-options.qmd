...
- **Multi-tenant cloud** — Multiple organizations (tenants) share infrastructure while keeping data isolated, providing cost-efficiency and scalability. Private link connection support allows secure network connections.

- **{{< var validmind.vpv >}} (VPV)** — A single-tenant architecture where one organization uses a dedicated infrastructure instance, offering greater control, customization, and enhanced security. This option is ideal for high-compliance needs and also supports private link connection for secure connectivity.
