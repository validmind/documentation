...
### SR 26-2: Interagency Guidance on Model Risk Management

The interagency guidance issued by the Board of Governors of the Federal Reserve System, the Federal Deposit Insurance Corporation, and the Office of the Comptroller of the Currency in the United States in **2026** **supersedes SR 11-7**. It sets expectations for a **risk-based** model risk management program: a **narrowed** definition of models, **materiality** based on exposure and purpose, **ongoing monitoring and outcomes analysis** alongside validation, and a **comprehensive enterprise model inventory**.
...
SR 26-2 is widely recognized as the current U.S. interagency benchmark for model risk management practices.

SR 26-2 emphasizes these themes:

Model scope and materiality
: - Apply MRM to *complex quantitative methods* that use statistical, economic, or financial theory — and **tier** oversight by **materiality** (exposure and purpose).
- Identify and mitigate risks from incorrect or inappropriate model use, outputs, or implementation errors.
- Maintain **effective challenge** through independent validation and clear accountability.
...
Model Validation
: - Remain an integral part of managing model risk, ensuring models perform as intended for their materiality tier.
...
The guidance mandates **ongoing monitoring**, **outcomes analysis**, and **periodic reviews** so models remain fit for use as conditions change.
...
### Meeting regulatory requirements with {{< var vm.product >}}
 ...
 As a robust tool for implementing Model Risk Management (MRM) best practices, including the _three lines of defense_, {{< var vm.product >}} significantly aids organizations in adhering to the regulatory guidelines set forth by SR 26-2 and SS1/23. 
...
: {{< var vm.product >}} offers a suite of tools for model developers, facilitating thorough documentation and rigorous testing of models, aligning with the regulatory expectations of both SR 26-2 and SS1/23, particularly for models under regulatory purview.
...
Model inventory
: The {{< var vm.product >}} **{{< fa cubes >}} Inventory** encapsulates a centralized repository for all models, aiding in streamlined tracking, management, and monitoring, simplifying compliance with the inventory mandates specified in SR 26-2 and SS1/23.

:::

::: {.w-50-ns}
Lifecycle management and custom workflows
: {{< var vm.product >}}’s capabilities extend to effective model lifecycle management through configurable workflows. This structured approach to managing model risks across various lifecycle stages significantly aids in meeting the rigorous management and oversight expectations set by SR 26-2 and SS1/23.
...
: With automated validation features and comprehensive risk assessment tools, {{< var vm.product >}} aligns with the effective validation criteria and thorough risk evaluation mandates of SR 26-2 and SS1/23.
...
By integrating these features, {{< var vm.product >}} provides a comprehensive suite of tools that not only simplifies the path to compliance with SR 26-2 and SS1/23 but also embeds a culture of rigorous and transparent model risk management within the organization.
