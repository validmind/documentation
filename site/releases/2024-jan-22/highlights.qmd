...
- **Fix support for OpenAI library**. We have updated our demonstration notebooks for large language models (LLMs) to provide correct support for `openai >= 1.0.0`. Previously, some notebooks were using an older version of the OpenAI client API.
...
- **Removed deprecated high-level API methods**: The API methods `run_template` and `run_test_plan` had been deprecated previously. They have now been removed from the the ValidMind Developer Framework.
  
  Developers should update their code to use the recommended high-level API methods:
