...
You can now pass a dataset with pre-computed model predictions. By default, the developer framework will compute model predictions when calling `init_model`. 

In the following example we will try to call the `predict()` method of the model for the training and test datasets:
