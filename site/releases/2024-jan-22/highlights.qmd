...
#### `init_model` improvements

To improve performance and efficiency, you can now pass a dataset with pre-computed model predictions when initializing a model. By default, if no prediction column is specified when calling `init_model`, the ValidMind Developer Framework will compute the model predictions as before.

To illustrate how using prediction columns can help, consider the following example _without a prediction column_: 
...
Internally, this example invokes the `predict()` method of the model for the training and test datasets when the model is initialized. The issue here is that this approach can be problematic with large datasets: `init_model` can simply take too long to compute.
...
Usage example _with a prediction column_:
...
The addition of a `feature_columns` argument allows you to specify a list of feature names for prediction. Internally, the function filters the dataset to retain only these specified features for prediction-related tasks, leaving the remaining dataset available for other purposes, such as segmentation. This improvement replaces the existing behavior of `init_dataset`, which loaded the entire dataset, incorporating all available features for prediction tasks. While this approach worked well, it could impose limitations when generating segmented metrics and proved somewhat inefficient with large datasets containing numerous features, of which only a small subset were relevant for prediction.
...
Additionally, two new metrics enable you to report by segment and show the use of `feature_columns`:

- TO DO LINK? `MissingValuesBySegment`
- TO DO LINK? `DescriptiveStatisticsBySegment`

A new notebook illustrates the configuration of dataset features:
...
Try it yourself: TO DO LINK [Configure data set features](../../notebooks/how_to/configure_dataset_features.ipynb)
...
#### `run_documentation_tests()` improvements

The `run_documentation_tests()` function, used to collect and run all the tests associated with a template, now supports running multiple sections at a time. This improvement addresses a request we received through customer feedback. The previous behavior was to allow running only one section at a time. This change maintains backward compatibility with the existing syntax, requiring no updates to your code.

Example usage to run one section:
...
Example usage to run multiple sections:
...
Try it yourself: TO DO LINK [Run documentation sections](../../notebooks/how_to/run_documentation_sections.ipynb)
...
#### Support for arbitrary and custom inputs <span id="arbitrary-test-new">

The ValidMind Developer Framework now supports passing arbitrary or custom inputs as an `inputs` dictionary when running individual tests or test suites. This support replaces the standard inputs `dataset`, `model`, and `models`, which are now deprecated.
...
To make it easier for you to adopt arbitrary and custom inputs, we have updated our how-to notebooks and code samples to use the new recommended syntax:
...
- How-to notebooks, including:
  ...
  - TO DO LINK [Configure data set features](../../notebooks/how_to/configure_dataset_features.ipynb)
...
Also check [Standard inputs are deprecated](#arbitrary-test-legacy).
...
- **Support for OpenAI library >=1.0**. We have updated our demonstration notebooks for large language models (LLMs) to provide the correct support for `openai >= 1.0.0`. Previously, some notebooks were using an older version of the OpenAI client API.

## Deprecations

<!---feat: [sc-2468] Allow arbitrary test context (V2) by @johnwalz97 in [#312](https://github.com/validmind/developer-framework/pull/312)--->
- **Standard inputs are deprecated**. <span id="arbitrary-test-legacy"> The ValidMind Developer Framework now supports passing arbitrary or custom inputs as an `inputs` dictionary when running individual tests or test suites. As a result, the standard inputs `dataset`, `model`, and `models` are deprecated and might be removed in a future release. If you are a developer, you should update your code to use the new, recommended syntax.
  ...
  Also check [Support for arbitrary and custom inputs](#arbitrary-test-new).
