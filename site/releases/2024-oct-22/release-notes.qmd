...
--->
### Add monitoring section to user guides
...
This update introduces significant enhancements to the documentation by adding a comprehensive section on monitoring capabilities. The new 'Monitoring' section includes detailed guides on how to enable monitoring for models and review ongoing monitoring results in the ValidMind Platform UI. Instructions are provided for adding `monitoring=True` to code snippets and selecting a monitoring template, along with steps to access and review results. 

The glossary now features new terms related to monitoring, with definitions for concepts like backtesting, model drift, model performance, ongoing monitoring, recalibrating models, and reporting and governance. To aid visual understanding, images and examples of monitoring results have been added.

In the test descriptions section, updates include ongoing monitoring tests such as Feature Drift, Prediction Across Each Feature, Prediction Correlation, and Target Prediction Distribution Plot. Code samples now demonstrate how to enable and use monitoring within the platform. Additionally, minor updates were made throughout the documentation to ensure consistency and clarity regarding these new capabilities.
...
Labels: enhancement
--->
### Doc: Archive and delete inventory models

You can now archive models

When you decommission models or delete models that you no longer need, you can archive them. This helps keep your ValidMind inventory accurate and up to date with your organization's current resources.
 
Generated PR summary: 
 
This update introduces new documentation to help you manage models within the model inventory. The guide, `archive-delete-models.qmd`, provides detailed instructions on archiving decommissioned models and deleting those that are no longer needed. You'll find prerequisites and step-by-step instructions for changing a model's stage, archiving, activating, and deleting models. Additionally, the `_quarto.yml` file has been updated to include this guide in the navigation, and the `managing-model-inventory.qmd` document now references this comprehensive new resource.
...
Added a new use case for bias and fairness analysis
...
- `application_scorecard_with_bias.ipynb`

<!--- REPLACE THIS COMMENT WITH YOUR DESCRIPTION --->
 
Generated PR summary: 
 
The latest update to the ValidMind Developer Framework introduces new features and improvements that enhance bias and fairness testing. You can now install new dependencies `aequitas`, `fairlearn`, and `vl-convert-python` through updated GitHub Actions workflows, crucial for bias analysis. A new Jupyter notebook, `application_scorecard_with_bias.ipynb`, guides you in building a credit risk model with integrated bias and fairness analysis using ValidMind. Accompanying this, a dataset called `lending_club_biased.csv.gz` and its processing script have been added for testing purposes, alongside an update to the dataset module initializer.

To evaluate model fairness more effectively across protected classes, new test modules are introduced: `ProtectedClassesCombination.py`, `ProtectedClassesDescription.py`, `ProtectedClassesDisparity.py`, and `ProtectedClassesThresholdOptimizer.py`. Existing test modules have been refactored for improved code quality. Lastly, there is an additional parameter `class_of_interest` in the SHAPGlobalImportance tool for better SHAP value selection. These updates are reflected in the version increase from 2.5.17 to 2.5.18.
...
Labels: enhancement
--->
### Mechanism for filtering columns

<!--- COMMENT --->

When running a test, you have the option to filter specific columns in your dataset. You can do this to focus on relevant data. This feature helps simplify and analyze your results more efficiently. Remember, you can select which columns to include in your analysis based on your criteria

Support for column filtering when running tests

Tests can run on a subset of columns in a dataset without creating a new dataset with columns removed. See the following example to learn how to run the `DatasetDescription` test with only a single column from a dataset:
...
See this [example notebook](https://github.com/validmind/developer-framework/blob/main/notebooks/how_to/input_column_filtering.ipynb) for more information.
...
This release enhances the `testing-overview.qmd` documentation by embedding a YouTube video. This video serves as a visual guide, making it easier for you to understand the available tests and test suites. The video is embedded using the `{{< video >}}` shortcode with specified width and height attributes to ensure optimal viewing.
