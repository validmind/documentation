...
This release includes a number of improvements to the developer experience when using the ValidMind Developer Framework, along with a visual redesign of the Platform UI.
...
- **Enhancements to test configuration discovery**: Making the discovery of required context and default configuration easier is key to improving the user experience, and this update is a significant step in that direction. The following changes have been made to how `required_context` and `default_parameters` are surfaced:
