...
::: {.g-col-8}
Our new end-to-end notebook gives you a full introduction to the {{< var validmind_developer >}}. You can use this notebook to learn how the end-to-end documentation process works, based on common scenarios you encounter in model development settings. 
:::
...
1. **Initializing the {{< var validmind_developer >}}** — The {{< var developer >}} provides tools for documentation and testing, from dataset descriptions to model validation using various open-source frameworks.
...
Since there's no model object available, the {{< var developer >}} won't be able to call `model.predict()` or `model.predict_proba()`. You need to load predictions and probabilities manually. For example:
...
::: {.g-col-8}
![](documentation-overview-page.png)
:::
...
{{< video business-unit-risk-areas.mp4 width="600" >}}
...
::: {.g-col-8}
![](template-yaml.png){width="60%" fig-alt="A screenshot showing template YAML"}<br><br>
![](duplicate-template-id.png){width="60%" fig-alt="A screenshot showing the error message 'Duplicate section ID test found in template'"}
:::
