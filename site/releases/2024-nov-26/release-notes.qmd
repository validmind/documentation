...
#### Replace developer framework with library

Replace all references  
  
To replace all mentions of "developer framework" with "Library," follow these steps. First, ensure you locate every instance of "developer framework." Replace each occurrence with the term "Library." After updating each reference, double-check your document to confirm that all instances have been changed. Save your work after making these adjustments.
 
Generated PR summary: 
 
This update replaces the term 'Developer Framework' with 'Library' across various components in your project to ensure consistent terminology. The changes affect user interface text in components such as `ActivityFeedWidget`, `AddBlockMetricOverTime`, `AddBlockTestDrivenModal`, `ModelDocumentation`, `DataExploration`, and `GettingStarted`. These modifications are textual and do not impact the functionality of the components.
...
#### Updated product terms docs side*
...
<!-- New homepage & search:
https://github.com/validmind/documentation/pull/465 -->

#### ValidMind Academy 

<!-- Training style overhaul:
...
--->
##### Rework training section

Update validator training

We introduced a new training homepage  

The new training homepage makes it easier for you to find and register for courses. We also expanded our validator training and user guide. It includes more guidance for tracking and resolving issues identified in model findings. [Training](https://docs.validmind.ai/training/training.html){.button}
...
{{< var vm.product >}} offers several specialized features that use large language models (LLMs). The features streamline model risk management and ensure regulatory compliance. Here's how we approach these features and what you need to know:

[Large language model features]( /about/overview-llm-features.qmd)
 
Generated PR summary: 
 
This update enhances the preview functionality and introduces new features and documentation to the project. The CSS for the `.preview` class has been adjusted to allow for more flexible sizing by removing fixed dimensions, while the Lua script for previews now accepts optional `width` and `height` attributes with default values of 400 and 225, respectively. It can also determine if URLs are external and adjust accordingly, generating HTML content that allows the iframe to scale fully within its container.

Additionally, new documentation is available for large language model (LLM) features. This includes a detailed overview file that covers aspects such as test interpretation, risk assessment, qualitative checks, and document checker functionalities while emphasizing using the company's products for risk management and compliance. The site navigation menu now includes a link to this documentation.

New media assets, including images and a GIF, have been added to support the LLM features documentation. These enhancements improve preview flexibility and provide comprehensive information on LLM capabilities.
...
<!-- New colors & logo:
https://github.com/validmind/documentation/pull/366
https://github.com/validmind/documentation/pull/524 -->
