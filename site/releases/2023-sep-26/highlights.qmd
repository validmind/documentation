...
- **Prompt validation demo notebook for LLMs**. As a proof of concept, we added initial native prompt validation tests to the Developer Framework. It also adds a demo notebook and simple template to test out these metrics on the Sentiment Analysis LLM model we have built.
...
- **Simplified developer experience when running tests.** We added a new `run_test` helper function that streamlines running tests for you. This function allows executing any individual test independent of a test suite or a documentation template. A one-line command can execute a test, making it easier to run tests with various parameters and options. 
   ...
   **Example usage of `run_test`:**
   ...
   ![](270865328-67488af6-0f19-4d81-bf50-3dc95d0fb305.png)
    ...
    Using the test ID, run a given test and pass additional configuration parameters and inputs to it:
