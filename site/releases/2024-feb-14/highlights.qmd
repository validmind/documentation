...
#### Documentation templates now support multiple unique results for the same metric

Documentation templates have been updated to support logging multiple unique results for the same test. This support for allows you to run the same test across different datasets or models. To make use of this new feature, you add a unique `result_id` identifier as a suffix to a `content_id` identifier in the content block definition of a `metric` or `test` content type. 
...
In the developer framework, each of these unique `content_id` identifiers can be configured by passing the appropriate `config` and `inputs` in `run_documentation_tests()` or `run_test()`. 

For example, to configure two separate tests for `Skewness` using different datasets and parameters:
...
#### Specify inputs for tests individually with `run_documentation_tests()`

The `run_documentation_tests()` function has been updated to allow passing test `inputs` and `params` via the `config` parameter.

Previously, `config` could already pass `params` to each test that gets declared:
...
In this example, the test `SomeTest` receives a custom value for the param `min_threshold`.

With the updated function, `config` can now pass both `params` and `inputs` to each declared test. For example, you can now specify what model should be passed to each individual test instance:
...
Here, the top-level `inputs` parameter acts as a _global inputs_ parameter, and the individual tests can customize what they see as _input model_ via their own `config` parameters.
...
#### See available task types and tags
...
- New `list_tasks_and_tags()` endpoint enable you to list which `tags` are associated to which `task_type`
...
#### Developer framework documentation inputs
...
#### Update events to show users who ran the developer framework actions

Update rendering of events related to the developer framework. We are now showing the user who ran the action instead of a generic "Developer Framework" actor name
...
We simplified the instructions for getting started with the ValidMind Developer Framework in the Platform UI. These instructions tell you how to use the code snippet for your documentation project with your own model or with one of our code samples.
...
Model owner can edit model details page
...
Reduced load times for the ValidMind Platform UI
: We reduced the loading times of our Platform UI to improve the user experience.
...
: Enhanced the Model Inventory filtering to allow filtering by Developers and Validators.
...
: The model inventory has been updated to allow organizations to add custom fields. This enhancement enables administrators to customize the model inventory according to your specific organizational needs.
...
: Fixed an issue where using an invalid test identifier would prevent `run_documentation_tests()` from running all available tests. The full test suite now runs as expected, even when an invalid test identifier is specified.
...
: Our user guide now includes end-to-end instructions for managing users on the ValidMind platform. This new content covers common tasks such as inviting new users, adding them to user groups, and managing roles and permissions. [Learn more ...](../../guide/user-management.qmd)
