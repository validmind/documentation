...
Documentation templates have been updated to support logging each metric run as a unique result, making it possible to run same test across different datasets or models. To make use of this new feature, you add a unique `result_id` identifier as a suffix to a `content_id` identifier in the content block definition of a `metric` or `test` content type. 
