...
An important addition to the {{< var validmind_developer >}}, you can now run comparison tests with the developer framework. This feature is useful when you want to run the same test against multiple combinations of models or datasets. A comparison test creates a single documentation block that compares the individual results.
...
Users with the `Create_Role` permission can now add a new role under Settings {{< fa arrow-right >}} Roles: [Work with roles in the {{< var validmind_platform >}}](https://app.prod.validmind.ai/settings/roles)
...
[Try our reports in the {{< var validmind_platform >}}](https://app.prod.validmind.ai/reports)
