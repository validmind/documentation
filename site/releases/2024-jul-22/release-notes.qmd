...
With enhancements to the `VMDataset` class, the [`add_extra_column`](/validmind/validmind/vm_models.html#VMDataset.add_extra_column) method now supports adding 2D arrays as single columns in the dataset's DataFrame. These changes ensure that 2D arrays are stored in a single column without altering the DataFrame structure. 
