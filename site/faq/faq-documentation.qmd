...
Yes, the {{< var validmind.platform >}}[^2] allows you to configure versioned templates based on document requirements for each record (model) or lifecycle use case.
...
## Can I run tests and log documentation without a record (model)?
<span id="no-available-model"><span>
Yes! If you do not have a record ready, or your record can't be loaded directly, or you only have access to predictions, you can still run tests and log documentation using the {{< var validmind.developer >}} as long as you're able to load the predictions.
