...
ValidMind currently allows multiple users to simultaneously edit documentation in the {{< var vm_platform >}}. If two users are editing the same cell on the UI, the most recently saved version of the content will prevail. 
...
ValidMind supports disconnected workflows natively at the data-collection level since the {{< var vm_dev >}} creates individual test runs every time a new test iteration is executed. This allows for running parallel/disconnected tests that individually send results to the ValidMind API.
...
You can also leverage ValidMind’s {{< var vm_dev >}} once you are ready to document a specific model for review and validation. That is, you do not need to use ValidMind while you are in the exploration or R&D phase of model development.
