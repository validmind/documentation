...
For distributed training pipelines built with frameworks like TensorFlow, ValidMind can directly access the trained model instance to extract metadata stored in the model object, if the {{< var vm.developer >}} is imported from within the pipeline's code. ValidMind can also ingest metrics and test results from the customer's training or evaluation pipeline, using batch prediction or online prediction mechanisms.
