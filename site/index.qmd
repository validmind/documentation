...
async function loadSearchIndex() {
    const response = await fetch(SEARCH_INDEX_URL);  // Fetch the JSON file
    const searchData = await response.json();  // Parse the JSON into an object
    return searchData;  // Return the parsed JSON data
}

// Function to initialize Lunr.js and index your documents for searching
async function setupLunr() {
    const searchData = await loadSearchIndex();  // Load search.json data

    // Create Lunr.js index
    const idx = lunr(function () {
        // Define the fields to index
        this.ref('href');  // The unique reference field (URL)
        this.field('title');  // Index the title field
        this.field('content');  // Index the content field

        // Add each document from searchData to the index
        searchData.forEach(function (doc) {
            this.add(doc);  // Add document to the index
        }, this);  // 'this' refers to the Lunr.js instance
    });

    return { idx, searchData };  // Return both the index and the raw data
}

// Call setupLunr to initialize the search index and prepare it for queries
setupLunr().then(({ idx, searchData }) => {
    document.getElementById('searchbox').addEventListener('input', function (event) {
        const query = event.target.value;  // Get the current search query from the input field
        const results = idx.search(query);  // Perform a search with Lunr.js

        const hitsContainer = document.getElementById('hits');  // Get the container for displaying search results
        hitsContainer.innerHTML = '';  // Clear previous search results

        // Iterate over the search results
        results.forEach((result) => {
            // Find the original document by matching the href (result.ref) with the searchData
            const doc = searchData.find(d => d.href === result.ref);

            // Check if doc exists to avoid errors
            if (doc) {
                const resultElement = document.createElement('div');
                // Add the title and a preview of the content (first 100 characters)
                resultElement.innerHTML = `<a href="${doc.href}"><strong>${doc.title}</strong><br>${doc.text.substring(0, 100)}...</a>`;
                hitsContainer.appendChild(resultElement);  // Append the result to the hits container
            } else {
                console.warn(`Document not found for href: ${result.ref}`);
            }
        });
    });
});
