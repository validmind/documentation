...
- On the first run, the workflow waits for a preview site on the first run. For subsequent runs, it checks the latest available site, which may be behind HEAD. The PR comment shows which commit SHA was checked — rerun the check if needed.
